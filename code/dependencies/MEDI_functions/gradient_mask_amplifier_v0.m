...
% for k = 1:zm 
    for i = 3:xm-2
        for j = 3:ym-2
            ...
            %kernel = iMag(i-2:i+2,j-2:j+2,k-2:k+2);
            %kernel = [iMag(i-2,j+2,k) iMag(i-1,j+2,k) iMag(i,j+2,k) iMag(i+1,j+2,k) iMag(i+2,j+2,k);
                      %iMag(i-2,j+1,k) iMag(i-1,j+1,k) iMag(i,j+1,k) iMag(i+1,j+1,k) iMag(i+2,j+1,k);
                      %iMag(i-2,j,k)   iMag(i-1,j,k)   iMag(i,j,k)   iMag(i+1,j,k)   iMag(i+2,j,k);
                      %iMag(i-2,j-1,k) iMag(i-1,j-1,k) iMag(i,j-1,k) iMag(i+1,j-1,k) iMag(i+2,j-1,k);
                      %iMag(i-2,j-2,k) iMag(i-1,j-2,k) iMag(i,j-2,k) iMag(i+1,j-2,k) iMag(i+2,j-2,k)];
            %if iMag_g(i,j,k)<sc
            if kernel < sc
                iMag_g(i,j,:,:) = iMag_g(i,j,:,:).*dgrad;
            end
        end
    end
end
