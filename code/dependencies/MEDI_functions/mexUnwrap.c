...
inline static float myround (float num){
...
inline static void get_neighbors(int *nei,int *pos,int *dim)
{
...
inline static void heap_add(point *ori_image, int * heap, int index, int * length)
{
int ind, parent, goon;
...
inline static int heap_pop( point * ori_image, int * heap, int * length)
{
...
*/		
		...
		if (ori_image[heap[0]].pre >=0) 
			ori_image[heap[0]].phase = ori_image[heap[0]].phase - myround( ( ori_image[heap[0]].phase - ori_image[ ori_image[heap[0]].pre ].phase )/(2*PI) )   *2*PI;
