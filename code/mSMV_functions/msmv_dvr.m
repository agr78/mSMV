...
function msmv_vr(in_file,out_file,max_vr,smv_r,dr)
    ...
    SphereK_dr = single(sphere_kernel(matrix_size,voxel_size,dr));
    % Partition mask
    Mne = SMV(Mask,SphereK_dr) > 0.999;
    Me = Mask-Mne;
    ...
    if exist('R2s','var') == 1
        Mv = imbinarize(fibermetric((Mask_ev.*R2s),[1:max_vr],'ObjectPolarity','bright'),0);
        Mb = Mb == 1 & Mv == 0;
    else
        Mv = zeros(size(Mb));
    end
