...
function msmv(in_file,out_file)
    ...
    if ~contains(in_file,'sim')
        ...
        Mask_ves = (abs(Mask.*filt)>prctile(abs(filt),95)).*MaskErode(Mask,matrix_size,voxel_size,3); 
%         
...
%         disp('Applying vessel mask')
        ...
        % Vessel filter
%         N_stdm = N_std.*Mask;
%         Mask_ves = (N_stdm<prctile(N_stdm(Mask>0),98) & N_stdm>prctile(N_stdm(Mask>0),75));
        % Preserve the vessels that extend to the edge
        SE = strel("diamond",1);
        Mask_ves = imdilate(Mask_ves,SE);
        Mask_bk = Mask_bk == 1 & Mask_ves == 0;
        disp('Applying vessel mask')
    end
    ...
    disp('+/-'); disp(pne)
    RDF_s = RDF_s0;
    RDF_s = (~Mask_bk).*RDF_s+SMV(Mask_bk.*RDF_s0,matrix_size,voxel_size,5);
